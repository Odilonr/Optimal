...
def fill():
    if expanded: # If the frame is exanded
        ...
        home_b.config(image=home,font=(0,19))
        set_b.config(image=settings,font=(0,19))
        ring_b.config(image=ring,font=(0,19))
maison = Image.open('images/home_icon.png').resize(30,30)
# Define the icons to be shown and resize it
home = PhotoImage(maison)
settings = PhotoImage(file='images/profile_icon.png')
ring = PhotoImage(file='images/menu_icon.png')
...
home_b = Button(frame,image=home,bg='orange')
set_b = Button(frame,image=settings,bg='orange')
ring_b = Button(frame,image=ring,bg='orange')
